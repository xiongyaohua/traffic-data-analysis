...
\begin{frame}
    ...
    \begin{frame}
        ...
        $\vec{\theta}$取什么值时$L(\theta)=\vec{e}^T\vec{e}$最小，这个最小值记为。
    ...
    \begin{frame}
        \frametitle{取偏导数}
        将$\vec{e}=\mat{A}\vec{\theta}-\vec{b}$代入公式\ref{eq:least_square}有
        \begin{equation}
            \begin{split}
                L(\vec{\theta})&=\vec{e}^T\vec{e}\\
                &={(\mat{A}\vec{\theta}-\vec{b})}^T(\mat{A}\vec{\theta}-\vec{b})\\
                &=\vec{\theta}^T\mat{A}^T\mat{A}\vec{\theta}+\vec{b}^T\vec{b}-\vec{\theta}^T\mat{A}^T-\vec{b}^T\mat{A}\vec{\theta}
            \end{split}
        \end{equation}
        对$\vec{\theta}$取\emph{偏导数}有
        \begin{equation}
            \frac{\partial L(\Vt)}{\partial\Vt}=
            2\MA^T\MA\Vt-2\MA^T\Vb
        \end{equation}
    \end{frame}
    ...
    \begin{frame}
        ...
        回到方程$\MA\Vt=\Vb$，假设$\MA$只有两列，将方程写成\emph{列向量}形式
        \begin{equation}
    ...
    \begin{frame}
        \frametitle{情况一：有唯一解}
        \begin{itemize}
            \item 当向量$\Vb$在向量$\Va_1$、$\Va_2$张开的平面上时，$\Vt$有唯一解。
            \item $\theta_1$、$\theta_2$分别是$\Vb$在$\Va_1$、$\Va_2$方向上的\emph{投影长度}
            \begin{equation}
                \theta_i = \Vb\cdot\frac{\Va_i}{\|\Va_i\|}
            \end{equation}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            \item 当向量$\Vb$、$\Va_1$、$\Va_2$在同一个方向上时，$\Vt$有无穷多解。
            \item 所有解满足条件
            \begin{equation}
                \theta_1\|\Va_1\|+\theta_1\|\Va_1\|=\|\Vb\|
            \end{equation}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            \item 当向量$\Vb$在向量$\Va_1$、$\Va_2$张开的平面外时，$\Vt$无解。
            \item 此时\emph{核心思路}是，我们可以在平面上挑选最\emph{接近}$\Vb$的向量$\Vb^*$。
            方程$\MA\Vt=\Vb^*$属于情况一、二，保证有解。
            \item 最小二乘法选取$\Vb$在平面上的投影作为$\Vb^*$。
        \end{itemize}
    \end{frame}

    \begin{frame}
        \frametitle{从投影到最小二乘法}
        令投影点$\Vb^*$到$\Vb$之间的向量为$\Ve$，有$\Vb^*+\Ve=\Vb$，或者
        \begin{equation}
            \Ve=\Vb-\Vb^*
        \end{equation}
        由于$\Vb^*=\MA\Vt^*$，$\Ve$正好是\emph{残差}。
