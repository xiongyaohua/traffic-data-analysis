...
为了求解$a$、$b$的近似值，我们可以用平面内的点$\Vv^*$代替$\Vv$，两者的误差是$\Ve=\Vv-\Vv^*$。为了尽量准确我们要求$\Vv^*$是平面上离$\Vv$最近的一点，可以证明$\Vv^*$是$\Vv$在平面上的垂直投影点，也即是说$\Ve$与平面垂直。
参考\cref{fig:ls-vectors}帮助理解。
参考\cref{fig:ls-vectors1}帮助理解。

\begin{figure}[h]
    \ContinuedFloat* %必须，不然编号出错，见caption宏包
    \sidecaption{线性回归的几何意义是找到目标矢量$\Vv$在由$\Vk$和$\vec{1}$张开平面上的投影点$\Vv^*$}
