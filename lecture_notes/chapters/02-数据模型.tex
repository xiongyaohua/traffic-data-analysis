...
，写作$\Vv$；中间是一个权重(weight)矩阵，写作$\mat{W}$；右边是由所有参数构成的矩阵，写作$\Vu$。此时\cref{eq:la-kv}进一步简化表示为
\begin{equation}
    \Vv=\mat{W}\cdot\mat{u}
\end{equation}
...
当线性方程组$\Vv=\mat{W}\cdot\mat{u}$因为过约束无解时，方程两边同时乘以$\mat{W}^{-1}$，将\cref{eq:la-eq}转化为以下\emph{最小二乘形式}
\begin{equation}\label{eq:least-square}
    \mat{W}^{-1}\Vv=\mat{W}^{-1}\mat{W}\cdot\mat{u}
\end{equation}
此时方程是否有解由矩阵$\mat{W}^{-1}\mat{W}$是否可逆决定。
可以证明无论$\mat{W}$是什么形状，$\mat{W}^{-1}\mat{W}$都是正方形矩阵，一般来说可逆。
最佳近似解，为了与精确解区别记作$\hat{\mat{u}}$\sidenote{$\hat{\Vu}$读作"hat u"}，可以通过以下公式计算
\begin{equation}
    \hat{\Vu}=(\mat{W}^{-1}\mat{W})^{-1}\mat{W}^{-1}\Vv
\end{equation}
...
\begin{equation}\label{eq:e-vec}
    ...
    \end{split}
\end{equation}
其中$\Vv$是所有速度实测值的向量，$\mat{W}$是权重矩阵，$\Vk$是所有密度数据的向量，$\vec{1}$是全$1$向量。

把\cref{eq:e-vec}带入\cref{eq:sse-vec}，然后展开化简可以得到
\begin{equation}
    \begin{split}
        \text{SSE(a,b)} &= \Ve^T\cdot\Ve\\
        &=(\Vv-\mat{W}\Vu)^T(\Vv-\mat{W}\Vu)\\
        &=\Vv^T\Vv+(\mat{W}\Vu)^T\mat{W}\Vu
        -\Vv\mat{W}\Vu-(\mat{W}\Vu)^T\Vv\\
        &=\Vv^T\Vv+\Vu^T\mat{W}^T\mat{W}\Vu
        -\Vv\mat{W}\Vu-\Vu^T\mat{W}^T\Vu
    \end{split}
\end{equation}