...
\begin{frame}
  \frametitle{损失函数的梯度}
      线性回归的损失函数是
      \begin{equation*}
        L(\Vt)=(\MA\Vt-\Vy)^T(\MA\Vt-Vy)
      \end{equation*}
      对$\Vt$求偏导有
      \begin{equation*}
        \boxed{\nabla_{\Vt}L(\Vt)=\MA^T\MA\Vt-\MA^T\Vy}
      \end{equation*}

      Ridge回归的损失函数是
      \begin{equation*}
        L(\Vt)=(\MA\Vt-\Vy)^T(\MA\Vt-Vy)+\lambda\Vt^T\Vt
      \end{equation*}
      对$\Vt$求偏导有
      \begin{equation*}
        \boxed{\nabla_{\Vt}L(\Vt)=(\MA^T\MA+\lambda\MI)\Vt-\MA^T\Vy}
      \end{equation*}
...
\begin{frame}
  ...
  \begin{equation*}
    ...
    &=-\frac{1}{n}\sum_{i=1}^n[y_i\cdot\log(\hat{y}_i)+(1-y_i)\cdot\log(1-\hat{y_i})]
    \end{split}
  \end{equation*}
  进一步整理成矩阵形式有
  \begin{equation}\label{eq:logistic_loss}
    \boxed{-\frac{1}{n}(\Vy^T\cdot\log(\sigma(\MA\Vt))+(1-\Vy)^T\cdot\log(\vec{1}-\sigma(\MA\Vt)))}
  \end{equation}
